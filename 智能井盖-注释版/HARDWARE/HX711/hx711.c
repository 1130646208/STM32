...
u8 hx711_Condition;//压力是否异常
u8 hx711_data[N];//存放压力值
u8 hx711_init_val=0;//初始压力值，用来校准
/*
    功能：hx711硬件初始化
    返回值：无
*/
...
/*
    功能：获取C传感器数据 时钟端口A5 数据端口A4
    返回值：AD转换值
*/

u8 get_hx711_data()//根据IIC时序图写代码
{
...
/*
    功能：校准压力值，由于每次系统运行压力都有一个初始值，所以需要记录这个初始值并用来校准
    返回值：无
*/
void hx711_calib()
{
...
void STRESS_Display(const u8* stress)
{
    ...
    OLED_ShowString(43,37,"   ",12);//刷新动态显示区
