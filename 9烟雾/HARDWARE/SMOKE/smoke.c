...
float gas_level_status()// 浓度状态判断函数，对数据进行判断，当浓度达到临界值时，返回0，正常返回1。
{
...
{
    OLED_Fill(0,24,64,48,0);//刷新动态显示区
